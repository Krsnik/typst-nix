...
pkgs.stdenvNoCC.mkDerivation {
  ...
  buildPhase = ''
    ${if timings then "mkdir -p $out/dev" else "mkdir $out"}
    ...
    --ignore-system-fonts \
    ${
      if fonts != [ ] && format != "html" then
        ''--font-path "${strings.concatStringsSep ":" fonts}"''
      else
        ""
    } \
    ${
    ...
    ${if timings then "--timings $out/dev/timings.json" else ""} \
    --features html \
    "$out/${name}${
      if
